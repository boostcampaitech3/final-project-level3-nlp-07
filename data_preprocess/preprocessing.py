...
def preprocessing_cafelist(input_fname="nonpre_total_cafes.csv", output_fname="pre_total_cafes.csv"):
    ...
    masking_username = lambda x: x.str.replace(r'[ㄱ-ㅎ가-힣\w]{0,}\*\*님', '[고객이름]님', regex=True) # 영어**님 -> [고객 이름]님
    masking_username = lambda x: x.str.replace(r'[ㄱ-ㅎ가-힣\w]{0,}\*\*고객님', '[고객이름]고객님', regex=True)
    masking_cafename = lambda x, cafe: re.sub(f"{cafe.split('-')[0]}", '[상호명]', x) # 카페이름 -> [상호명]

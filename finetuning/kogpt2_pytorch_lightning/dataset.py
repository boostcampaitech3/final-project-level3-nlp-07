...
class KoGPTSummaryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.prompt_length > 0:
            article = self.tok.encode(self.bos_token) + self.tok.encode(PTUNING) * self.prompt_length + self.tok.encode(instance['고객리뷰'])
        else:
        ...
        summary = self.tok.encode(self.summary_token) + self.tok.encode(instance['사장답글']) + self.tok.encode(self.eos_token)
